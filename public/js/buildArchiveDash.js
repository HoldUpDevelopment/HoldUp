...
$(document).ready(async function () {
    active = fetch(`${origin}/user/getActiveUserRole`, {
        ...
        routeList = fetch(`${origin}/route/getArchivedRoutes`, {
            method: "GET",
            headers: {
                "Authorization": `Bearer ${sessionStorage.getItem('jwt')}`
            }
            ...
            routes.forEach(async route => {
                response = await fetch(`${origin}/route/getRouteDetails?routeId=${route._id}&isArchived=true`, {
                    method: "GET"
                })
                ...
                routeObjectHTML = `<tr id="${route._id}">
                        ...
                        <li><a id="unarchive${count}" class="dropdown-item" href="#">Unarchive route</a></li>
                ...
                //Save the route data we are obtaining to construct the modals
                ...
                $(`#unarchive${count}`).unbind("click").on('click', async function (event) {
                    id = $(this).closest("tr").attr("id");
                    var response = await fetch(`${origin}/route/unarchiveRoute`, {
                        method: "POST",
                        headers: {
                            "Authorization": `Bearer ${sessionStorage.getItem('jwt')}`,
    ...
    $('#deleteRouteButton').unbind("click").on('click', async function (event) {
        ...
        if (target == undefined) {
            ...
            console.log(target)
            var response = await fetch(`${origin}/route/deleteArchivedRoute?routeId=${target}`, {
                method: "DELETE",
                headers: {
                    "Authorization": `Bearer ${sessionStorage.getItem('jwt')}`
                }
